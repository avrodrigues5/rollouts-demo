  ...
  stages {
    ...
    stage('Build') {
      ...
      steps {
        container('docker') {
          // Build new image
          sh "until docker ps; do sleep 3; done && docker build -t avrodrigues5/rollouts-demo:${env.GIT_COMMIT} ."
          // Publish new image
          sh "docker login --username $DOCKERHUB_CREDS_USR --password $DOCKERHUB_CREDS_PSW && docker push avrodrigues5/rollouts-demo:${env.GIT_COMMIT}"
        }
      }
    }
    ...
    stage('Deploy Canary') {
      ...
      steps {
        container('tools') {
          ...
          sh "git clone https://$GIT_CREDS_USR:$GIT_CREDS_PSW@github.com/avrodrigues5/rollouts-demo-deployment.git"
          ...
          dir("rollouts-demo-deployment") {
            ...
            sh "kustomize edit set image avrodrigues5/rollouts-demo:${env.GIT_COMMIT}"
